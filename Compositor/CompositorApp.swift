        ...
        Window("Compositor", id: "editor") {
            ...
            .commands {
                ...
                CommandMenu("Layer") {
                    ...
                    Button(L10n.text(session.selection == nil ? "Duplicate Layer" : "Layer via Copy")) { session.layerViaCopy() }
                    ...
                    Divider()
                    Button(L10n.text(session.activeLayer?.maskSourceID == nil ? "Create Clipping Mask" : "Release Clipping Mask")) {
                    ...
                    Button(L10n.text(session.activeLayer?.isVisible == false ? "Show Layer" : "Hide Layer")) {
                    ...
                    Button("Move Layer Down") { session.moveActiveLayer(by: -1) }
                        .configuredKeyboardShortcut("[").disabled(!session.canMoveActiveLayer(by: -1))
                    Group {
                        Button(L10n.text(session.mergeTitle)) { session.mergeLayers() }
                    ...
                    Divider()
                    Button(session.selectedEffect != nil ? L10n.format("Delete %@", L10n.text(session.selectedEffect!.kind.rawValue)) : L10n.text(session.isMaskSelected && session.activeLayer?.mask != nil ? "Delete Layer Mask" : session.selectedLayerIDs.count > 1 ? "Delete Layers" : "Delete Layer")) {
