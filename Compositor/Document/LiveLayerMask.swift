...
extension EditorSession {
    ...
    func deleteWithLiveMaskChoice(_ ids: [UUID]) -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = localizedString(ids.count == 1 ? "This layer supplies a live mask" : "These layers supply live masks")
        alert.informativeText = localizedString("Bake keeps the current masked appearance in the dependent layers’ pixels. Remove Links reveals their pixels. You can undo either choice.")
        alert.addButton(withTitle: localizedString("Bake and Delete"))
        alert.addButton(withTitle: localizedString("Cancel"))
        alert.addButton(withTitle: localizedString("Remove Links and Delete"))
