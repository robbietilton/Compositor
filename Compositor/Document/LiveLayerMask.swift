...
extension EditorSession {
    ...
    @discardableResult func linkMask(source: UUID, target: UUID) -> Bool {
        ...
        beginEdit(L10n.string("Create Clipping Mask"))
    ...
    func removeLiveMask(from target: UUID) {
        ...
        let releases = siblings[(targetIndex...)]
            ...
            .map(\.id)
        beginEdit(L10n.string("Release Clipping Mask"))
        for id in releases {
...
extension EditorSession {
    ...
    func deleteWithLiveMaskChoice(_ ids: [UUID]) -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = ids.count == 1 ? L10n.string("This layer supplies a live mask") : L10n.string("These layers supply live masks")
        alert.informativeText = L10n.string("Bake keeps the current masked appearance in the dependent layers’ pixels. Remove Links reveals their pixels. You can undo either choice.")
        alert.addButton(withTitle: L10n.string("Bake and Delete"))
        alert.addButton(withTitle: L10n.string("Cancel"))
        alert.addButton(withTitle: L10n.string("Remove Links and Delete"))
    ...
    func finishDeletingLayer(_ id: UUID, baked: [UUID: ImportedImage]) {
        ...
        beginEdit(L10n.string("Delete Layer"))
    ...
    func finishDeletingLayers(_ ids: [UUID], baked: [UUID: ImportedImage]) {
        ...
        beginEdit(L10n.string("Delete Layers"))
