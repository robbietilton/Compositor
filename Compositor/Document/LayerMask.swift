...
nonisolated struct LayerMask: Equatable, @unchecked Sendable {
    ...
    static func solid(revealing: Bool) -> LayerMask? {
        ...
        return LayerMask(asset: ImportedImage(image: image, thumbnail: image, name: L10n.string("Layer Mask")))
    }
    static func asset(from image: CGImage) throws -> ImportedImage {
        ...
        return ImportedImage(image: image, thumbnail: thumbnail, name: L10n.string("Layer Mask"))
    }
...
extension EditorSession {
    ...
    func addMask(revealing: Bool = true) {
        ...
        do {
            ...
            finishOpacityEdit()
            beginEdit(L10n.string("Add Mask from Selection"))
    ...
    func addLayerMask(revealing: Bool = true) {
        ...
        finishOpacityEdit()
        beginEdit(revealing ? L10n.string("Add Reveal-All Mask") : L10n.string("Add Hide-All Mask"))
        document?.layers[index].mask = mask
        isMaskSelected = true
        endEdit()
    }
    func toggleLayerMask() {
        guard canEditMask, activeLayer?.mask != nil,
              let index = document?.layers.firstIndex(where: { $0.id == activeLayerID }) else { return }
        finishOpacityEdit()
        beginEdit(activeLayer?.mask?.isEnabled == true ? L10n.string("Disable Layer Mask") : L10n.string("Enable Layer Mask"))
    ...
    func deleteLayerMask() {
        guard canEditMask, activeLayer?.mask != nil,
              let index = document?.layers.firstIndex(where: { $0.id == activeLayerID }) else { return }
        finishOpacityEdit()
        beginEdit(L10n.string("Delete Layer Mask"))
    ...
    func copyMask(from source: UUID, to target: UUID) {
        ...
        beginEdit(layers[index].mask == nil ? L10n.string("Copy Layer Mask") : L10n.string("Replace Layer Mask"))
        document?.layers[index].mask = mask
        selectLayer(target)
        isMaskSelected = true
        endEdit()
    ...
    func toggleMaskLink(_ id: UUID) {
        ...
        commitTransform()
        finishOpacityEdit()
        beginEdit(mask.isLinked ? L10n.string("Unlink Layer Mask") : L10n.string("Link Layer Mask"))
    ...
    func commitMaskTransform(_ edit: TransformEdit) {
        ...
        if let corners = edit.corners {
            ...
            do {
                ...
                finishOpacityEdit()
                beginEdit(L10n.string("Distort Layer Mask"))
        ...
        finishOpacityEdit()
        beginEdit(L10n.string("Transform Layer Mask"))
