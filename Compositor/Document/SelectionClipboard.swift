...
extension EditorSession {
    ...
    func paste() {
        ...
        if let clip = pixelClipboard, pasteboard.changeCount == clip.changeCount {
            addPixelLayer(clip.image, at: clip.origin, name: nextLayerName(), editName: L10n.string("Paste"))
            ...
            addPixelLayer(image, at: origin, name: nextLayerName(), editName: L10n.string("Paste"))
        } else { NSSound.beep() }
    }
    ...
    func layerViaCopy() {
        ...
        do {
            guard let copied = try renderSelectedPixels(from: layer, mask: isMaskSelected) else { NSSound.beep(); return }
            addPixelLayer(copied.image, at: copied.region.origin, name: nextLayerName(), editName: L10n.string("Layer via Copy"))
        } catch { brushError = error.localizedDescription }
    }
    ...
    func duplicateActiveLayer() {
        ...
        let copy = ImageLayer(id: UUID(), asset: layer.asset, name: L10n.format("%1$@ copy", layer.name), isVisible: layer.isVisible,
    ...
    func nextLayerName() -> String {
        ...
        return L10n.format("Layer %1$@", String(number))
    }
