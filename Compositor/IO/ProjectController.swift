...
final class ProjectController {
    ...
    func exportPNG() async {
        ...
        panel.title = localizedString("Export PNG")
        panel.nameFieldStringValue = (session.projectURL?.deletingPathExtension().lastPathComponent ?? localizedString("Untitled")) + ".png"
    ...
    func canvasSize() async {
        ...
        let options: CanvasSizeOptions? = await withCheckedContinuation { continuation in
            ...
            sheet.title = localizedString("Canvas Size")
    ...
    func imageSize() async {
        ...
        let options: ImageSizeOptions? = await withCheckedContinuation { continuation in
            let sheet = NSWindow()
            sheet.styleMask = [.titled, .fullSizeContentView]
            sheet.title = localizedString("Image Size")
    ...
    func exportJPEG() async {
        ...
        do {
            ...
            let data: Data? = await withCheckedContinuation { continuation in
                let sheet = NSWindow()
                sheet.styleMask = [.titled, .fullSizeContentView]
                sheet.title = localizedString("Export JPEG")
            ...
            panel.canCreateDirectories = true
            panel.isExtensionHidden = false
            panel.title = localizedString("Export JPEG")
            panel.nameFieldStringValue = (session.projectURL?.deletingPathExtension().lastPathComponent ?? localizedString("Untitled")) + ".jpg"
    ...
    private func saveCurrent(asNew: Bool = false) async -> Bool {
        ...
        if destination == nil {
            ...
            panel.canCreateDirectories = true
            panel.isExtensionHidden = false
            panel.nameFieldStringValue = session.projectURL?.lastPathComponent ?? localizedString("Untitled.comp")
            panel.title = localizedString(asNew ? "Save Project As" : "Save Project")
            let response: NSApplication.ModalResponse
            if let window { response = await panel.beginSheetModal(for: window) }
            else { response = await panel.begin() }
    ...
    func open(_ suppliedURL: URL? = nil) async -> Bool {
        ...
        if source == nil {
            ...
            panel.title = localizedString("Open Project")
            let response: NSApplication.ModalResponse
            if let window { response = await panel.beginSheetModal(for: window) }
            else { response = await panel.begin() }
            guard response == .OK, let url = panel.url else { return false }
    ...
    private func confirmReplacement() async -> Bool {
        ...
        let alert = NSAlert()
        alert.messageText = String(format: localizedString("Save changes to %@?"), session.projectURL?.lastPathComponent ?? localizedString("Untitled"))
