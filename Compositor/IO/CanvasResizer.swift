    ...
    func resize(_ snapshot: ProjectSnapshot, to options: CanvasSizeOptions) throws -> ProjectSnapshot {
        ...
        if let color = options.fill, options.width > old.width || options.height > old.height {
            ...
            let asset = try autoreleasepool {
                ...
                return ImportedImage(image: image, thumbnail: thumbnail, name: L10n.string("Canvas Extension"))
            }
            let id = UUID()
            images[id] = asset
            manifest.layers.insert(ProjectLayerRecord(id: id, name: L10n.string("Canvas Extension"), isVisible: true,
