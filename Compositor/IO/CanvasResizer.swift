    ...
    func resize(_ snapshot: ProjectSnapshot, to options: CanvasSizeOptions) throws -> ProjectSnapshot {
        ...
        if let color = options.fill, options.width > old.width || options.height > old.height {
            ...
            let asset = try autoreleasepool {
                ...
                return ImportedImage(image: image, thumbnail: thumbnail, name: "Canvas Extension".localized)
            }
            let id = UUID()
            images[id] = asset
            manifest.layers.insert(ProjectLayerRecord(id: id, name: "Canvas Extension".localized, isVisible: true,
