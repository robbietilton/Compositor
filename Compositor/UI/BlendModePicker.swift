...
struct BlendModePicker: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSPopUpButton {
        ...
        button.addItems(withTitles: LayerBlendMode.allCases.map { L10n.string($0.rawValue) })
    ...
    final class Coordinator: NSObject, NSMenuDelegate {
        ...
        func menu(_ menu: NSMenu, willHighlight item: NSMenuItem?) {
            let mode = item.flatMap { item in
                let index = menu.index(of: item)
                return LayerBlendMode.allCases.indices.contains(index) ? LayerBlendMode.allCases[index] : nil
            }
        ...
        @objc func choose(_ button: NSPopUpButton) {
            guard session.activeLayerID == layerID,
                  let mode = highlightedMode ?? LayerBlendMode.allCases.enumerated().first(where: { $0.offset == button.indexOfSelectedItem })?.element else { return }
