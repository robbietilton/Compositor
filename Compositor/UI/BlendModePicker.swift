...
struct BlendModePicker: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSPopUpButton {
        ...
        for (index, group) in LayerBlendMode.groups.enumerated() {
            ...
            for mode in group {
                // Titles are localized for display; the raw value rides along for lookup.
    ...
    final class Coordinator: NSObject, NSMenuDelegate {
        ...
        func menu(_ menu: NSMenu, willHighlight item: NSMenuItem?) {
            ...
            guard let mode = item.flatMap({ LayerBlendMode(rawValue: $0.representedObject as? String ?? $0.title) }) else { return }
        ...
        @objc func choose(_ button: NSPopUpButton) {
            guard session.activeLayerID == layerID,
                  let mode = highlightedMode ?? button.selectedItem.flatMap({ LayerBlendMode(rawValue: $0.representedObject as? String ?? $0.title) }) else { return }
