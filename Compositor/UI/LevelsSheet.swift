...
struct LevelsSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text(localized(session.adjustmentOriginal != nil ? "Underlying pixels · alpha-weighted histogram" : session.selection == nil ? "Original pixels · alpha-weighted histogram" : "Original pixels · selection and alpha-weighted histogram"))
                .font(.caption).foregroundStyle(.secondary)
            Divider()
            HStack {
    ...
    private func field(_ name: String, _ binding: Binding<Double>, decimals: Int) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 5) {
            Text(localized(name)).font(.caption).foregroundStyle(.secondary)
                .scrubbable(sensitivity: decimals == 0 ? 1 : 0.01, value: binding, range: range)
            TextField(localized(name), value: binding, format: .number.precision(.fractionLength(decimals)))
    ...
    private var histogram: some View {
        ...
        }.accessibilityLabel(String(localized: "Original \(localized(settings.channel.rawValue)) histogram"))
