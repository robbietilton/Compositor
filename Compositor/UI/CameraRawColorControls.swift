...
struct CameraRawCurveControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Picker("Curve", selection: Binding(get: { edit?.cameraRawCurvePage ?? .parametric }, set: { session.filterEdit?.cameraRawCurvePage = $0 })) {
                ForEach(CameraRawCurvePage.allCases, id: \.self) { Text(localized($0.rawValue)).tag($0) }
            }
            .pickerStyle(.segmented)
            .labelsHidden()
            .help(localized("Parametric lifts tonal regions. Point places anchors on the curve."))
            if edit?.cameraRawCurvePage == .point {
                Picker("Channel", selection: Binding(get: { edit?.cameraRawPointChannel ?? .rgb }, set: { session.filterEdit?.cameraRawPointChannel = $0 })) {
                    ForEach(CameraRawPointChannel.allCases, id: \.self) { Text(localized($0.rawValue)).tag($0) }
                }
                .pickerStyle(.segmented)
                .labelsHidden()
                .help(localized("RGB changes brightness. Red, green, and blue also shift the color."))
            }
            curveGraph
                .frame(height: 150)
            ...
            if edit?.cameraRawCurvePage != .point {
                ...
                if let point = selectedPoint {
                    Text("In \(Int((point.x * 255).rounded()))   Out \(Int((point.y * 255).rounded()))")
                        ...
                        .help(localized("Input and output of the selected curve point."))
                }
                ...
                .help(localized("Replaces this curve with a straight line or a contrast curve."))
    ...
    private func slider(_ title: String, _ key: WritableKeyPath<CameraRawCurveSettings, Double>, _ range: ClosedRange<Double>, _ reset: Double, _ help: String) -> some View {
        HStack {
            Text(localized(title)).frame(width: 88, alignment: .leading).help(localized(help))
                .scrubbable(sensitivity: 1,
                            value: Binding(get: { raw.curve[keyPath: key] },
                                           set: { value in update { $0.curve[keyPath: key] = value } }), range: range)
            CameraRawSlider(value: raw.curve[keyPath: key], range: range, track: .plain, help: localized(help),
                            onChange: { value in update { $0.curve[keyPath: key] = value } },
                            onReset: { update { $0.curve[keyPath: key] = reset } })
            TextField(localized(title), value: Binding(get: { raw.curve[keyPath: key] }, set: { value in update { $0.curve[keyPath: key] = value } }),
                      format: .number.precision(.fractionLength(0...0)))
                .frame(width: 48).help(localized(help))
        }
    }
...
struct CameraRawMixerControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Picker("Mixer", selection: Binding(get: { edit?.cameraRawMixerPage ?? .hsl }, set: { session.filterEdit?.cameraRawMixerPage = $0 })) {
                ForEach(CameraRawMixerPage.allCases, id: \.self) { Text(localized($0.rawValue)).tag($0) }
            }
            .pickerStyle(.segmented)
            .labelsHidden()
            .help(localized("HSL lists every color. Color edits one family. Point Color adjusts a color you pick."))
            ...
            case .hsl:
                Picker("Component", selection: Binding(get: { edit?.cameraRawMixerTab ?? .hue }, set: { session.filterEdit?.cameraRawMixerTab = $0 })) {
                    ForEach(CameraRawMixerTab.allCases, id: \.self) { Text(localized($0.rawValue)).tag($0) }
                }
                .pickerStyle(.segmented).labelsHidden()
                .help(localized("Hue shifts the color, Saturation its strength, and Luminance its brightness."))
            ...
            .help(localized("Drag a color in the picture. Nearby color families move together."))
        }
    }
    ...
    private func colorSlider(_ title: String, _ key: WritableKeyPath<CameraRawMixerSettings, [Double]>, _ help: String) -> some View {
        ...
        return HStack {
            Text(localized(title)).frame(width: 88, alignment: .leading).help(localized(help))
            CameraRawSlider(value: raw.mixer[keyPath: key][index], range: -100...100, track: familyTrack(index, key), help: localized(help),
    ...
    private var pointColor: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                .help(localized("Click the picture to save a color. Up to eight colors."))
                ...
                ForEach(raw.mixer.points.indices, id: \.self) { index in
                    ...
                    .buttonStyle(.plain)
                    .help(localized("Select this picked color."))
                }
            }
            if raw.mixer.points.indices.contains(edit?.cameraRawPointIndex ?? 0) {
                ...
                Toggle("Visualize Range", isOn: Binding(get: { raw.mixer.points[edit?.cameraRawPointIndex ?? 0].visualize },
                    ...
                    .help(localized("Dims the picture outside this color's range. It is not kept when you press OK."))
            }
        }
    }
    ...
    private func pointSlider(_ title: String, _ key: WritableKeyPath<CameraRawPointColor, Double>, help: String,
        ...
        return HStack {
            Text(localized(title)).frame(width: 110, alignment: .leading).help(localized(help))
            CameraRawSlider(value: raw.mixer.points[index][keyPath: key], range: range, track: track, help: localized(help),
...
struct CameraRawGradingControls: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Picker("Grading", selection: Binding(get: { page }, set: { session.filterEdit?.cameraRawGradePage = $0 })) {
                ForEach(CameraRawGradePage.allCases, id: \.self) { Text(localized($0.rawValue)).tag($0) }
            }
            .pickerStyle(.menu)
            .labelsHidden()
            .fixedSize()
            .help(localized("Three-Way shows shadows, midtones, and highlights. The other choices show one wheel."))
            if page == .threeWay {
                HStack(spacing: 30) {
    ...
    private func wheel(_ title: String, _ key: WritableKeyPath<CameraRawGradingSettings, CameraRawGradeWheel>) -> some View {
        ...
        return VStack(spacing: 4) {
            Text(localized(title)).font(.caption).help(localized("Drag inside the wheel. Angle sets hue, distance sets saturation."))
            ...
            Text("\(Int(wheel.hue.rounded()))°  \(Int(wheel.saturation.rounded()))")
                ...
                .help(localized("Hue and saturation of this wheel."))
    ...
    private func slider(_ title: String, _ value: Double, _ range: ClosedRange<Double>, _ reset: Double, _ help: String, set: @escaping (Double) -> Void) -> some View {
        HStack {
            Text(localized(title)).frame(width: 78, alignment: .leading).help(localized(help))
            CameraRawSlider(value: value, range: range, track: .plain, help: localized(help), onChange: set, onReset: { set(reset) })
        }
    }
...
private struct GradeWheel: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            .help(localized("Drag to set hue and saturation. Double-click to reset this wheel."))
        }
    }
}
