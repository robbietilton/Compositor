    ...
    @ViewBuilder private var toolHeaders: some View {
        Group {
            ...
            if session.tool == .gradient {
                if session.fillToolMode == .bucket { PaintBucketControls(session: session) }
                else { GradientControls(session: session) }
                Divider()
            }
