...
struct ContentView: View {
        ...
        .toolbar {
            ToolbarItem(placement: .navigation) {
                Button { requestNewCanvas() } label: { Label("New canvas", systemImage: "plus") }
                    .help(localized("New canvas (⌘N)")).accessibilityIdentifier("newCanvasToolbar")
            ...
            ToolbarItem(placement: .primaryAction) {
                Button("Fit") { session.fit() }.help(localized("Fit canvas in window (⌘0)"))
                    .accessibilityIdentifier("fitCanvas").disabled(session.document == nil)
                    .padding(.horizontal, 4)
            }
            ToolbarItem(placement: .primaryAction) {
                Button("100%") { session.zoom(to: 1) }.help(localized("Actual pixels (⌘1)"))
            ...
            ToolbarItem(placement: .primaryAction) {
                HStack(spacing: 0) {
                    ...
                    }.help(localized("Zoom in (⌘+)")).disabled(session.document == nil)
                    Button { session.zoomKeyboard(by: -1) } label: {
                        Image(systemName: "minus.magnifyingglass")
                    }.help(localized("Zoom out (⌘−)")).disabled(session.document == nil)
                }
                .padding(.horizontal, 4)
            }
        }
            ...
            ForEach(NavigationTool.allCases.filter { $0 != .idle }, id: \.self) { tool in
                Button { session.selectTool(tool) } label: {
                    Group {
                        ...
                        // `textformat` localizes its glyph (格式) in a Chinese-localized app; pin the Latin "Aa".
                        else if tool == .type { Image(systemName: session.symbol(for: tool)).font(.system(size: 17)).environment(\.locale, Locale(identifier: "en")) }
                ...
                .buttonStyle(.plain).help(localized(tool.label)).accessibilityLabel(localized(tool.label))
    ...
    /// One line of tool-specific cheat sheet in the status bar. Each branch is a catalog key.
    private var toolHint: String {
        switch session.tool {
        case .marquee:
            return localized(session.marqueeKind == .ellipse
        ...
        case .wand:
            return localized(session.wandMode == .object
        ...
        case .lasso:
            return localized(session.lassoKind == .freehand
        ...
        case .brush:
            return localized(session.brushMode == .erase ? "Drag to erase" : "Drag to paint")
                + localized(" · [ ] size · Shift-[ ] hardness · 1–0 opacity · Escape cancel · Space to pan")
        case .blur:
            return localized(session.blurMode == .blur ? "Drag to soften" : session.blurMode == .smudge ? "Drag to smudge" : "Drag to push pixels")
                + localized(" · [ ] size · Shift-[ ] hardness · 1–0 strength · Space to pan")
        case .cloneStamp:
            return localized("Option-click to set the source · Drag to clone · [ ] size · Shift-[ ] hardness · 1–0 opacity · Space to pan")
        case .spotHealing:
            return localized("Drag over blemishes to heal · [ ] size · Shift-[ ] hardness · Escape cancel · Space to pan")
        case .type:
            return localized("Drag a text box · Click text to edit · Drag box handles to resize · ⌘Return finish · Escape cancel")
        case .shape:
            return String(localized: "Drag to draw a shape on a new layer · Shift \(session.shapeKind == .line ? "45°" : session.shapeKind == .rectangle ? localized("square") : localized("circle")) · Option from center · Shift-U or Tab for the next shape · Escape cancel · Space to pan")
        case .gradient:
            return localized("Drag to draw · Drag ends to adjust · Shift 45° · 1–0 opacity · Enter apply · Escape cancel")
        case .crop:
            return localized("Drag to crop · Enter apply · Escape cancel · Space to pan")
        case .move:
            return localized("Drag to move · Handles to resize · Circle to rotate · 1–0 layer opacity · Space to pan")
        case .hand:
            return localized("Drag to pan · Pinch to zoom")
        case .idle:
            return localized("No tool selected · Press a tool's key to pick one · Space to pan")
        default:
            return localized("Click to zoom in · Option-click to zoom out · Drag right or left to zoom smoothly · Space to pan")
        }
    }
}
    ...
    var body: some View {
        Divider().overlay {
            Color.clear.frame(width: 8).contentShape(Rectangle())
                ...
                .help(localized("Drag to resize the panel"))
        }
    }
}
